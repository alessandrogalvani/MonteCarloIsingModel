...
double expmb=exp(-2./t);
   	...
   	const int Ly=Lx;
    const int N=Lx*Ly;
    const int nmontecarlo=2E6;
    const int steptermal=2E5;
    const int framerate=2E4; // after how many moves you take another screenshot of the lattice
...
double weight(int i, int j)
{
	double w=1.;
      ...
      w*=bondweight(m[i][j],m[next(i)][j]);
      w*=bondweight(m[i][j],m[previous(i)][j]);
    ...
    double magnx[Lx]={0};
         ...
         //   printlattice(lattice); cout<< endl;
          ...
          // cout<<localmean(lattice)<<endl;
		         if(mosse>steptermal&& mosse%framerate==0)
...
int main()
{
   srand (time(NULL));
   //("datiMCising.txt");
   //file.open("datiMCising.txt");
   //file.close();

   file.open("datiMCising.txt", ios::out | ios::trunc);
   file <<"t";
   for(int u=0;u<Lx*Ly; u++)
     {
       file<<"\t"<<"s"<<u;
     }
     file<<endl;
     ...
     //file.open("datiMCising.txt", std::ios_base::app);
     file.open("datiMCising.txt", std::ios_base::app);
   for (t=0.1; t < 10;  t+=0.4)
   {
     //cout<<"qq";
     beta=1./t;
     expb=exp(2./t);
